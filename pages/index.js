        ...
        <section className="container mx-auto px-4 py-12">
          ...
          <div className="flex flex-wrap justify-center gap-3 max-w-4xl mx-auto">
            {resume.skills.map((skill, i) => (
              <span 
                key={i}
                className="bg-blue-600/30 border border-blue-400 px-4 py-2 rounded-lg hover:bg-blue-600/40 transition"
              >
                {skill}
              </span>
            ))}
        ...
        <section className="container mx-auto px-4 py-12">
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            {resume.experience.map((exp, i) => (
              <div 
                key={i}
                className="bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20 hover:border-blue-400/50 transition"
              >
        ...
        <section className="container mx-auto px-4 py-12">
          ...
          <div className="max-w-4xl mx-auto grid md:grid-cols-2 gap-6">
            {resume.projects.map((project, i) => (
              <div 
                key={i}
                className="bg-white/10 backdrop-blur-sm rounded-lg p-6 border border-white/20 hover:border-purple-400/50 transition hover:scale-105"
              >
                ...
                {project.technologies && Array.isArray(project.technologies) && (
                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.technologies.map((tech, j) => (
                      <span 
                        key={j}
                        className="text-xs bg-blue-600/50 px-2 py-1 rounded"
                      >
                        {tech}
                      </span>
                    ))}
          ...
          <div className="max-w-4xl mx-auto space-y-4">
            {resume.education.map((edu, i) => (
              ...
              >
                ...
                {edu.field && (
                  <p className="text-gray-400 mt-1">Field: {edu.field}</p>
                )}
              </div>
            ))}
          </div>
